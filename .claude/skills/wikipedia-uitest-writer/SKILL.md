---
name: wikipedia-uitest-writer
description: Use when adding, updating, or expanding XCTest UI coverage in the Wikipedia iOS app's WikipediaUITests target. Writes journey-first UI tests with robots, deterministic launch configuration, fixture-backed defaults, and current Apple SDK guidance fetched through sosumi.ai.
---

# Wikipedia UI Test Writer
...
- Write test methods using the Robots pattern documented in the Robots.md
...
- Default to fixture-backed coverage with `fixture-strict`; use E2E only when live networking is truly required.
...
>Fixtures are captured from the live Wikipedia/MediaWiki APIs, not synthesized. Reproduce the request the app makes, including the app's iOS `User-Agent`, `Accept`, and `Accept-Language` headers, then save the exact response body under `WikipediaUnitTests/Fixtures`. Register the route in `WikipediaUnitTests/Fixtures/TestNetworkFixtures.json` with the request matcher, status, response headers, and `bodyResource`. Keep payloads byte-for-byte aligned with the API response; when `fixture-strict` mode hits an unregistered request, it returns a JSON 501 with the exact method and URL to add.
