...
This project's UI tests use the **Robots pattern**. Tests describe a user journey; selectors, waits, scrolling, and screenshot plumbing live in `WikipediaUITests/Robots`. Most failures are best fixed in a robot, an accessibility identifier, or a fixture — not in a test method or app code. See `WikipediaUITests/README.md` and `WikipediaUITests/ROBOTS.md`.
...
Always match the simulator and OS to how Githubn action CI is configured. Prefer a destination `id=` over a name/OS string once a matching simulator exists.
